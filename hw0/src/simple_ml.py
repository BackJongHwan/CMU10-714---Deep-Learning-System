...
def softmax_loss(Z, y):
    ...
    Z_exp = np.exp(Z)
    sum_exp = np.sum(Z_exp, axis = 1)
    log_sum_exp = np.log(sum_exp)

    m = y.shape[0]
    correct_class_scores = Z[np.arange(m), y]
    # Formula: log(sum(exp)) - z_y
    losses = log_sum_exp - correct_class_scores

    # Final step: Average over the batch
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    # if full batch..
    ''' 
    ### first get a raw score 
    # Z = softmax(X@theta)
    raw_score = np.exp(X@theta)
    row_sums = np.sum(raw_score, axis=1, keepdims=True)
    Z = raw_score / row_sums

    Iy = np.zeros((m, k))
    Iy[np.arange(m), y] = 1

    # result
    theta -= lr/batch * (X.T @ (Z - Iy))
    '''
    ...
    for i in range(0, m, batch):
        ...
        B = X_batch.shape[0]
        raw_score_batch = np.exp(X_batch @ theta)
        row_sums = np.sum(raw_score_batch, axis=1, keepdims=True)
        Z = raw_score_batch / row_sums


        Iy = np.zeros((B, k))
        Iy[np.arange(B), y_batch] = 1
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, m, batch):
        ...
        raw_score = np.exp(Z2)
        row_sums = np.sum(raw_score, axis=1, keepdims=True)
        S = raw_score / row_sums

        G2 = S - Iy
        G1 = (G2 @ W2.T) * (Z1 > 0)

        grad_W1 = (X_batch.T @ G1) / B
        grad_W2 = (Z1.T @ G2) / B
